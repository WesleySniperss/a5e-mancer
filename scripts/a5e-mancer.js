...
/* Pure system bugfix (no module UI involved), so it runs even when the module's
   `enable` setting is off. Only INSTALLS a lazy trigger here — the actual index
...
Hooks.once('ready', () => {
  ...
  /* Teaches Your Flavor about a5e's chat cards, which it otherwise skips
     ...
     Flavor has not booted yet; the bridge waits for its API itself. */
  YourFlavorService.init();
